...
class MatchGame extends Component {
  state = {categoryImg: 'FRUIT'  randimg:imagesList[Math.floor(Math.random() * imagesList.length)]}

  onUpdateId = id => {
    this.setState({categoryImg: id,randimg:imagesList[Math.floor(Math.random() * imagesList.length)]})
    
  }

  render() {
    const {categoryImg, randimg} = this.state
    console.log(categoryImg)
    const catWiseResults = imagesList.filter(
      each => each.category === categoryImg,
    )
      ...
      <div className="app-container">
        <nav className="navbar-container">
          <img
            ...
            alt="website logo"
          />
          <div className="sec-clock-container">
            ...
            <div className="clock-container">
              <img
                ...
                alt="timer"
              />
              <p className="time-count">0 sec</p>
            </div>
          </div>
        </nav>
        <div className="main-container">
          <ul>
            {randomObject.map(each => (
              <li key={each.id}>
                <img className="main-image" src={each.imageUrl} alt="imge" />
              </li>
            ))}
          </ul>
          <ul className="tab-container">
            {tabsList.map(each => (
              <li key={each.tabId}>
                <p
                  onClick={() => this.onUpdateId(each.tabId)}
                  className="tab-text"
                >
                  {each.displayText}
                </p>
          ...
          <ul className="ul-thumbinal-container">
            {catWiseResults.map(eachimg => (
              <li className="li">
                <img
